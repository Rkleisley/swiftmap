...
| `add_line` / `add_polyline` | Polylines | Long-format rows (`line_id_col`, `order_col`), WKT, GeoJSON, GeoPandas... A MultiLineString is one layer drawn as disjoint parts. `arrows=True` marks direction with contrast chevrons spaced evenly in screen pixels — the on-screen count holds at every zoom, and the end cap always shows (`arrows="end"` for caps only, `"segments"` for one per leg, `arrow_spacing=200` or `"2km"` to tune); `dash="8 4"` patterns the stroke |
