...
*   **Mixed collections:** `add_collection(data)` plots every geometry kind in one dataset — a GeoJSON `FeatureCollection`, a geostructures collection, or a GeoDataFrame with mixed geometry — adding one layer per kind, merged under a single sidebar entry. `add_geojson` and `add_geostructures` are aliases. Points render as circle markers; pass `point_type="markers"` for pins. Sources that hold one geometry kind per call (Pandas, Polars, coordinate lists) are rejected with a pointer to the specific method.
