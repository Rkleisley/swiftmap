...
*   `docs/` — **build output, not source.** The public demo site, generated by `scripts/build_demos.py` and served by GitHub Pages. See *Building the demo site*.
...
node scripts/check_demos.mjs         # every map, in a real browser
```

`docs/` is the public demo site, served by GitHub Pages from `/docs` on `main`
(Settings → Pages → Source: *Deploy from a branch*, branch `main`, folder `/docs`).
It is **generated, never edited**: `scripts/build_demos.py` runs swiftmap at build time
and writes one `map.json` plus its raw `.bin` buffers per map, and every code block on
the page is the exact string that was executed to build the map beside it — so a card
cannot drift from its example. Re-run it after any change that touches rendering, and
before a release.

The page is the *other* shape the host interface allows. `Map.save()` inlines the whole
bundle per file, which is right for handing someone one map and wrong for a page holding
twenty; `scripts/demo_assets/gallery.js` loads `assets/swiftmap.js` once and mounts each
map as a `createHostStub` over its `map.json`, driving exactly the five methods in
`src/host.js`. It is the only public consumer of that interface, which makes it a useful
canary: if the host contract slips, the demo site breaks before anyone's app does.

Two things worth knowing:

*   **`check_demos.mjs` is the acceptance test**, not the generator's exit code.
    "Twenty maps mounted in a browser and something reached the GPU" is the claim the
    page makes, and only a browser can answer it. `--shots <dir>` writes a screenshot per
    card for eyeballing; `--stacks` prints full stack traces for page errors.
*   **`docs/data/hero-1m/` is 20 MB** of float64 coordinates. It is deterministic (fixed
    seed), so it only changes when the generator does — but rewriting it repeatedly will
    grow the git history. `--skip-hero-large` builds only the smallest hero tier, which
    is what you want while iterating on anything else.
